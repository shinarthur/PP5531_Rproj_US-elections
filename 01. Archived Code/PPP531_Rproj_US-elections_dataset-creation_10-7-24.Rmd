...
# Left join court action status in 2012 to 2012on-wawrds subsetted data
treatedVAR.2012post.temp <- df_final.temp1 %>% 
  mutate(year_num = as.numeric(year)) %>%
  filter(year_num >= 2012) %>% 
  left_join(treatedVAR.2012.temp, by = join_by("state_abv" == "state_abv_VAR"),
            multiple = "all")

# Subset of pre-2012 (no treatment)
treatedVAR.2012pre.temp <- df_final.temp1 %>% 
  mutate(year_num = as.numeric(year)) %>% 
  filter(year_num < 2012)
  
# Bind Rows of pre-2012 & 2012-onwards
df_final.temp2 <- bind_rows(treatedVAR.2012post.temp, treatedVAR.2012pre.temp) %>% 
  arrange(state_abv, year_num) %>% 
  # Change pre-2012 binary treatment variable to 0
  mutate(across(redistrict_VAR_VAR:judiciary_VAR, ~case_when(is.na(.) ~ 0,
                                                             . >= 1 ~ 1,
                                                             TRUE ~ .)))
...
df_final.temp3 <- df_final.temp2 %>% 
  expss::apply_labels(year = "Election Cycle",
               ...
               statecrime.population = "No. of criminals in a state",
               statecrime.rates.property.all = "State's crime rates (ALL property-related crimes)",
               statecrime.rates.property.burglary = "State's crime rates (property: burglary crimes)",
               statecrime.rates.property.larceny = "State's crime rates (property: larceny crimes)",
               statecrime.rates.property.motor = "State's crime rates (property: motor crimes)",
               statecrime.rates.violent.all = "State's crime rates (ALL violent crimes)",
               statecrime.rates.violent.assault = "State's crime rates (violent: assault crimes)", 
               statecrime.rates.violent.murder = "State's crime rates (violent: murder crimes)",
               statecrime.rates.violent.rape = "State's crime rates (violent: rape crimes)",
               statecrime.rates.violent.robbery = "State's crime rates (violent: robbery crimes)",
               statecrime.totals.property.all = "State's no. of crimes (ALL property-related crimes)",
               statecrime.totals.property.burglary = "State's no. of crimes (property: burglary crimes)",
               statecrime.totals.property.larceny = "State's no. of crimes (property: larceny crimes)",
               statecrime.totals.property.motor = "State's no. of crimes (property: motor crimes)",
               statecrime.totals.violent.all = "State's no. of crimes (ALL violent crimes)",
               statecrime.totals.violent.assault = "State's no. of crimes (violent: assault crimes)",
               statecrime.totals.violent.murder = "State's no. of crimes (violent: murder crimes)",
               statecrime.totals.violent.rape = "State's no. of crimes (violent: rape crimes)",
               statecrime.totals.violent.robbery = "State's no. of crimes (violent: robbery crimes)",
