...
f.tidy.reg <- function(dv.sav = dv, iv.sav = iv, ctrl.sav = ctrl,
  ...
  tidyout <- tidy(out, conf.int = TRUE, conf.level = 0.9)
  return(tidyout)
}
...
### Creating Tidy Data
tidyout_NONE <- f.tidy.reg(dv.sav = dv, iv.sav = "interaction_court_action_none_count", ctrl.sav = ctrl) %>% 
  mutate(model_desc = "No court involvement",
         model = "Model A")

tidyout_ANY <- f.tidy.reg(dv.sav = dv, iv.sav = "interaction_judiciary", ctrl.sav = ctrl) %>% 
  mutate(model_desc = "ANY court involvement",
         model = "Model B")

tidyout_UNFAIR <- f.tidy.reg(dv.sav = dv, iv.sav = "interaction_judiciary_unfair", ctrl.sav = ctrl) %>% 
  mutate(model_desc = "Court finding: Unconstitutional map",
         model = "Model C")

tidyout_FAIR <- f.tidy.reg(dv.sav = dv, iv.sav = "interaction_judiciary_fair", ctrl.sav = ctrl) %>% 
