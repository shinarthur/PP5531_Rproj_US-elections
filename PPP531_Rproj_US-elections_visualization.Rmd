...
p.df <- read_rds("04. Outputs/2024-10-20_regressionDF.rds")
...
mean_vep <- p.df %>%
  group_by(year, judiciary_fair_map_VAR) %>%
  summarise(mean_vep = mean(log(vep_turnout_rate), na.rm = TRUE)) #%>% 
  filter(!(year == 2000))

ggplot(mean_vep, aes(x = year, y = mean_vep, color = judiciary_fair_map_VAR, group = judiciary_fair_map_VAR)) +
  geom_line(size = 1) +
  geom_point(size = 2)+
