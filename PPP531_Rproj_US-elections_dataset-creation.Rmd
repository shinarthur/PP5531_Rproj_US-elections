---
title: "U.S. Election Project | Dataset Creation"
...
## State Information
Creating a base dataframe using `usa::states` and a custom function. `usa::states` includes basic state-level information such as state name, abbreviation, fips code, area, etc. The custom function creates a panel dataframe, for all 50 states for each election year between 2000 and 2020. 
```{r base_df, include = TRUE}
...
## Redistricting Data
Redistricting data is then imported, processed, and merged with the previously created base dataframe.
```{r base_redist_df, include = TRUE}
...
### Merge redistricting data with previously created base_df
...
Datasets for each of the independent variables are imported and prepared to be merged with the base dataframe. The preparation step includes processing the data to be at a state-level. Variables included in this step include:

* Votes for House of Representative candidates
* Voter turnout rates
* Crime rates
* Unemployment rates
...
* GDP
* Wealth inequality
* Party control over state legislature

## Votes for House of Representative candidates
```{r df_housevote, include = TRUE}
...
## Unemployment rates
```{r df_unemp, results='hide', message=FALSE, include = TRUE}
...
## Wealth inequality
```{r df_wealth_inequality, include = TRUE}
...
# Merging Data
Independent variables are merged with the base dataframe.
...
# Additional Edits
This step includes the creation of binary treatment variables for creating the interaction terms in the data analysis step.

## Treatment Variables
```{r additional edits, include = TRUE}
...
## Labels
Labels are added to each variable for easier navigation.
```{r apply labels, include = TRUE}
...
# Export File
Dataset is exported as an RDS file. This file serves as the input for the other two scripts (reg-analysis & visualization.rmd)
...
# Further Data Edits
Step at the end in case further edits need to be made without disrupting the existing code.
```{r further edits, include = TRUE}
rm(list=ls())
