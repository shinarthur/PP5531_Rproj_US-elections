...
p.df <- df %>% 
  ...
  mutate(democrat_county_vote_perc = democrat_county_vote / county_totalvotes,
         ...
         time = case_when(year %in% c("2016") ~ 0,
                          year %in% c("2020") ~ 1),
         redistrict_count = case_when(redistrict_count == "1" | redistrict_count == "2" ~ 1,
                                      is.na(redistrict_count) ~ 0),
         interaction = time * redistrict_count,
         #redistrict_count = as.factor(redistrict_count),
         #time = as.factor(time),
...
### DID for pre-2020 and post-2020 [don't use]
